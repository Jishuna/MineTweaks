...
public class DownwardsLaddersTweak extends Tweak {
    ...
    public void reload() {
        FileUtils.loadResource(getOwningPlugin(), "Tweaks/Blocks/" + this.getName() + ".yml").ifPresent(config -> {
            ...
            this.maxDistance = config.getInt("ladder-place-distance", 5);
        });
    }
    ...
    private void onInteract(PlayerInteractEvent event) {
        ...
        if (block.getType() == Material.LADDER && item.getType() == Material.LADDER) {
            ...
            for (int distance = 0; target.getY() > block.getWorld().getMinHeight() && target.getType() == Material.LADDER && distance < this.maxDistance; distance++) {
